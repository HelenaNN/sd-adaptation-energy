...
BASE_OUTPUT_DIR = Path("calibration_outputs/lora")
GLOBAL_GPU_METRICS = BASE_OUTPUT_DIR / "global_gpu_metrics_calibration_lora.csv"

# Optional: pin specific GPU ID (set to None to use default environment)
GPU_DEVICE_ID = None  # e.g., "0"
...
GRAD_ACCUM_STEPS = 1
CHECKP_STEPS = 100
MAX_TRAIN_STEPS = 300  # Short training run to capture multiple checkpoints quickly
SEED = 1337
LEARNING_RATE = "1e-04"
COOLDOWN_SECONDS = 0
...
EXPERIMENTS = [
    ("dataset_sample", "A drawing of a tower."),
    ("dataset_sample", "A drawing of a tower."),
    ("dataset_sample", "A drawing of a tower."),
]

# ── Setup Directories & Global Log ───────────────────────────────────────────
...
def log_global_gpu_metrics(phase, exp_name, dataset, steps):
    cmd = ["nvidia-smi", "--query-gpu=temperature.gpu,utilization.gpu,memory.used,power.draw", "--format=csv,noheader,nounits"]
    if GPU_DEVICE_ID is not None:
        cmd.append(f"--id={GPU_DEVICE_ID}")
...
def start_global_gpu_monitor(exp_name, dataset, steps, interval_seconds=10):
    ...
    dev_arg = f"-i {GPU_DEVICE_ID} " if GPU_DEVICE_ID is not None else ""
    monitor_command = f"""
    while true; do
        timestamp=$(date +%s.%N)
        datetime=$(date --iso-8601=seconds)
        gpu_data=$(nvidia-smi {dev_arg}--query-gpu=temperature.gpu,utilization.gpu,memory.used,power.draw --format=csv,noheader,nounits)
...
for i, (dataset_name, validation_prompt) in enumerate(EXPERIMENTS):
    ...
    exp_name = f"{i}_calibration_lora_res{RESOLUTION}_{dataset_name}_steps{MAX_TRAIN_STEPS}_seed{SEED}"
