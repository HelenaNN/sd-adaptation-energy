...
BASE_OUTPUT_DIR = Path("/home/helena/Desktop/tfm/calibration_cp/sdv15/finetuning/calibration_a40")
GLOBAL_GPU_METRICS = BASE_OUTPUT_DIR / "global_gpu_metrics_calibration_a40_new_machine.csv"
...
def log_global_gpu_metrics(phase, exp_name, dataset, steps):
    result = subprocess.check_output(
        ["nvidia-smi", "--query-gpu=temperature.gpu,utilization.gpu,memory.used,power.draw",
         "--format=csv,noheader,nounits", "--id=0"], text=True).strip()
...
for i, (dataset_name, validation_prompt) in enumerate(EXPERIMENTS):
    ...
    exp_name = f"{i}_calibration_finetuning_a40_res{RESOLUTION}_{dataset_name}_steps{MAX_TRAIN_STEPS}_seed{SEED}"
