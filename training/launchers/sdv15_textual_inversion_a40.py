...
BASE_OUTPUT_DIR = Path("/home/helena/Desktop/tfm/experiments/sdv15/textual_inversion/outputs_a40_isolated_gpu")
GLOBAL_GPU_METRICS = BASE_OUTPUT_DIR / "global_gpu_metrics_a40_isolated_gpu.csv"
...
# Each entry: (dataset_name, max_train_steps, placeholder_token, initializer_token, validation_prompt)
EXPERIMENTS = [
    ("Battista", 5000, "<piranesi-style>", "art", "A drawing of a tower."),
    ("Battista", 5000, "<piranesi-style>", "art", "A drawing of a tower."),
    ("Battista", 5000, "<piranesi-style>", "art", "A drawing of a tower."),
]
...
def start_global_gpu_monitor(exp_name, dataset, steps, interval_seconds=10):
    ...
    while true; do
        ...
        gpu_data=$(nvidia-smi -i 0 --query-gpu=temperature.gpu,utilization.gpu,memory.used,power.draw --format=csv,noheader,nounits)
...
# ── Main loop ─────────────────────────────────────────────────────────────────
for i, (dataset_name, max_train_steps, placeholder_token, initializer_token, validation_prompt) in enumerate(EXPERIMENTS):

    dataset_dir = f"{DATASETS_BASE}/textual_inversion_{RESOLUTION}_{dataset_name}_dataset"

    exp_name = f"{i}_sdv15_textual_inversion_res{RESOLUTION}_{dataset_name}_steps{max_train_steps}_seed{SEED}"
