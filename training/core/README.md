...
* **Distributed Safety:** The tracker is initialized strictly on the local main process (`accelerator.is_local_main_process`) to prevent duplicate writes and race conditions during multi-GPU runs.
...
3. Once finished, `stop_task("checkpoint")` isolates and logs the energy consumed strictly during the saving process.
...
| `energy_per_checkpoint.csv` | Benchmarking the energy overhead of saving checkpoints |
